...
# Instanciation du Bot Telegram + suppression du webhook global
BOT = Bot(token=TELEGRAM_TOKEN)
try:
    BOT.delete_webhook(drop_pending_updates=True)
    print("✅ Webhook Telegram supprimé (global)")
except Exception as e:
    print(f"⚠️ Impossible de supprimer le webhook global : {e}")
...
def main_loop():
    ...
    while True:
        try:
            ...
            # Résumé quotidien 18h UTC
            if now >= next_summary:
                nb = len(positions)
                rest = MAX_TRADES_PER_MONTH - trades
                invested = trades * ETH_PER_TRADE
                summary = (
                    f"🧾 Résumé quotidien ({now:%Y-%m-%d}):\n"
...
def run_telegram_polling():
    ...
    # Suppression du webhook et purge des updates en attente
    loop.run_until_complete(
        app.bot.delete_webhook(drop_pending_updates=True)
    )

    # Démarrage du polling en dropant les anciennes mises à jour
